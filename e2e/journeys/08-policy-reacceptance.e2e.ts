...
test(
  'a new version of the agreement locks editing and nothing else',
  { tag: '@weekly' },
  async ({ browser, page, tracking }) => {
    try {
      ...
      await test.step('the agreement moves on', async () => {
        ...
        await expect(
          page.getByText(
            /This agreement has changed since you last accepted it/,
          ),
        ).toBeVisible();
      });
      ...
      await test.step('but what was published is still published', async () => {
        await anonymously(browser, async visitor => {
          ...
          await expect(
            visitor.getByText(SECTION, { exact: true }),
          ).toBeVisible();
        });
      });
      ...
      // A failure while the agreement is stale must not lock the next case,
      // or the retry, out of editing. Accepting is enough; the purge is the
      // teardown's.
      await tracking.openReady();
    }
  },

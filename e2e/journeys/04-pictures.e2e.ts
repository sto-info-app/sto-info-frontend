 ...
 *
 * This journey reaches services outside the machine it runs on: the file is
 * scanned by a third party and stored in Cloudflare Images, both against
 * whatever account the local configuration points at. So it is opt-in. Run it
 * with E2E_IMAGES=on when you mean to spend that. The external project runs
 * this file. The journeys project does not, so a full run does not upload it
 * twice.
 */
...
test(
  'a picture in each shape, replaced and removed',
  { tag: '@external' },
  async ({ page, tracking }) => {

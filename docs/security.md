...
| ----------------- | -------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `brace-expansion` | `5.0.7`        | Global | Multiple packages (Jest, ESLint, `@lhci/cli`, `serve`) pull in `brace-expansion@<5.0.7` (GHSA-3jxr-9vmj-r5cp — exponential-time DoS). 5.0.7 contains the fix.              |
...
```bash
npm ls brace-expansion esbuild js-yaml qs tmp uuid
npm audit --omit=dev
npm audit
```
...
Current expected audit state (last verified 2026-07-21):

- `npm audit --omit=dev`: `0 vulnerabilities`.
- `npm audit`: `0 vulnerabilities`.
- The active overrides now only cover dependencies that still need forced versions; removing any one of the six reintroduces an advisory.

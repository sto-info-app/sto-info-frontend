...
export interface ManagedActionHost {
...
/**
 * Sends a change to the server on behalf of a managed list, then reloads it.
 *
 ...
 *
 * Shared between features rather than owned by one. Storytime and Custom
 * Tracking both manage ordered lists the server is authoritative about, and
 * two copies of this would be two answers to the question of what a user is
 * told when a change is refused.
 *
 * Construct it in an injection context, so it can take the component's own
 * destroy, zone and change detection references.
 */
export class ManagedActionRunner {
   ...
   * @param _reload - How to reload the list once the server has accepted, for
   *   the callers whose action reports only that it worked. An action that
   *   answers with the new state itself passes nothing and settles the host in
   *   its own success handler, rather than asking for what it already has.
   * @param _failureMessage - What to say when the server explains nothing.
   */
  constructor(
    private readonly _host: ManagedActionHost,
   ...
   * @param onSuccess - Anything else to do once it succeeds, given whatever
   *   the action answered with.
   */
  run<T>(action: Observable<T>, onSuccess?: (result: T) => void): void {
    ...
    action
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          ...
          // generic apology when it can. Several complaints arrive as a list —
          // one per property a form got wrong — and a reader needs all of them
          // rather than whichever happened to be first.
          const reported = (
            error.error as { message?: string | string[] } | undefined
          )?.message;

          this._host.errorMessage = Array.isArray(reported)
            ? reported.join(' ')
            : (reported ?? this._failureMessage);

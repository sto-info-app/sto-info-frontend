...
/** Precomputed view model for a team member card in the developers list. */
...
export class TeamDevelopersComponent {
  ...
  /** Precomputed view models for current developer members. */
  readonly currentMemberVms: MemberVm[];

  /** Precomputed view models for past developer members. */
  ...
  getMemberLink(member: TeamMember): string {
    return `${this._routingService.getLink(APP_ROUTES.ABOUT_DEVELOPERS)}/${member.slug}`;
  }
